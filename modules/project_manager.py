...
class Projects:
    ...
    def get_current_time_block(self):
        current_time_block = self.db.query('select * from time_blocks '
                                           'where '
                                           'TIME(NOW()) between TIME(time_block_start) and TIME(time_block_end)')
        return current_time_block

    def get_current_task(self, time_block):
        current_task = self.db.query('select * from tasks where {} between task_block_start and task_block_end '
                                     'and DATE(NOW()) between task_start and task_end'.format(
            time_block))
